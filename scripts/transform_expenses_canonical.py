...
# 4) Cumulative 2025 (two series: cum_completed and cum_total)
con.execute(f"""
COPY (
  WITH m AS (
    SELECT date_trunc('month', tx_date) AS month,
           ...
           SUM(amount) AS total
    FROM read_parquet('{OUT}/expenses_canon.parquet')
    WHERE EXTRACT(YEAR FROM tx_date)=2025
    GROUP BY 1
  )
  SELECT month,
...
) TO '{OUT}/expenses_cumulative_2025.parquet' (FORMAT PARQUET);
""")
print("[expenses] wrote gold/expenses_cumulative_2025.parquet")
