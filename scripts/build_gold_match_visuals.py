...
try:
    ...
    print(f"[details] Completed rows parsed: {len(det_rows)} | months={len(completed)}")
except Exception as e:
    print(f"[details] WARN: {e}")
...
try:
    ...
    while i < len(lines):
        ...
        if "status" in low and "completed" in low:
            ...
            i = j; continue
        i += 1
    print(f"[status] Found status blocks: {len(blocks)}")
except Exception as e:
...
if blocks:
    if completed.empty:
        ...
        print("[status] No details found → choosing the widest status block")
    else:
        ...
        for b in blocks:
            ...
            if best_err is None or err < best_err:
                best_err = err; best = b
        chosen = best
        print(f"[status] Chosen block relative error vs details: {best_err:.6f}" if best_err is not None else "[status] Chosen block")
...
# ---------- Cumulative 2025
y2025 = bars[(bars["month"].apply(lambda d: getattr(d, "year", 0))==2025)].sort_values("month")
if not y2025.empty:
    comp = pd.Series(y2025["completed"].values, index=y2025["month"])
    proj = pd.Series(y2025["projected"].values, index=y2025["month"])
    out = pd.DataFrame({
        "month":        y2025["month"].values,
...
print("[gold] rebuilt (Completed from details or fallback-to-status; Projected from chosen status)")
